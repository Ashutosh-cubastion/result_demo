-- FUNCTION: public.sp_check_notallocation(text, integer)

-- DROP FUNCTION IF EXISTS public.sp_check_notallocation(text, integer);

CREATE OR REPLACE FUNCTION public.sp_check_notallocation(
	par_exam_name text,
...
create temp table checknotalloc
(
rollno text,
description text
);
	   ...
	   <<catsel_loop>>
		...
		if (last_merit>merit) then
			--if(dv_preference_post[i]='D65' and candidate_record.gender='1') then
				--insert into checknotalloc(rollno,description) values (candidate_record.rollno,'Female not allowed in D65 - keyvalue - '||catsel_category||dv_preference_post[i]||' Last Merit- '||last_merit||' candidate merit name - '||merit_name||' candidate merit- '||merit);
			--elsif
			if(dv_preference_post[i] in ('B19','B22','B31') and candidate_record.cat3 in ('4','5','7','8')) then
				insert into checknotalloc(rollno,description) values (candidate_record.rollno,'Pwbd not allowed in B19,B22,B31 - keyvalue - '||catsel_category||dv_preference_post[i]||' Last Merit- '||last_merit||' candidate merit name - '||merit_name||' candidate merit- '||merit);
				...
				insert into checknotalloc(rollno,description) values (candidate_record.rollno,'DEST Exempted not allowed. post code- '||dv_preference_post[i]||' catsel- '||catsel_category);
			else
				insert into checknotalloc(rollno,description) values (candidate_record.rollno,'Wrong Allocation- keyvalue - '||catsel_category||dv_preference_post[i]||' Last Merit- '||last_merit||' candidate merit name - '||merit_name||' candidate merit- '||merit);
			end if;
		end if;
...
copy checknotalloc to 'C:\Users\Public\CGLE2024_Allocation_Final\CheckNotAllocation\checknotalloc.csv' with csv header;
drop table checknotalloc;

return 'not allocation file generated';
exception when others then
	return SQLSTATE||','||SQLERRM;
end;
$BODY$;

ALTER FUNCTION public.sp_check_notallocation(text, integer)
    OWNER TO postgres;
