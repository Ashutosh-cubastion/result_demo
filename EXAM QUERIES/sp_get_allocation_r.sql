-- FUNCTION: public.sp_get_allocation_r(text, integer)

-- DROP FUNCTION IF EXISTS public.sp_get_allocation_r(text, integer);

CREATE OR REPLACE FUNCTION public.sp_get_allocation_r(
	par_exam_name text,
...
if((candidate_record.catsel_jso_18_32 is null or candidate_record.catsel_jso_18_32='')and (candidate_record.catsel_si_18_30 is null or candidate_record.catsel_si_18_30='')) then
	continue;
end if;
...
if (dv_preference_post[i]='C01') then
	catsel=candidate_record.catsel_jso_18_32;
	catsel_dob=candidate_record.catsel_dob_18_32;
elsif (dv_preference_post[i]='C02') then
	catsel=candidate_record.catsel_si_18_30;
	catsel_dob=candidate_record.catsel_dob_18_30;	
else
    continue;
end if;

length_of_catsel= length(catsel);
if(length_of_catsel is null or length_of_catsel='0') then 
	continue;
end if;----newly added
   <<catsel_loop>>
	for j in 1..length_of_catsel
		...
		if(vacancy_record.current>0) then
		  ...
		  UPDATE candidates SET allocated_category_r=catsel_category , allocated_post_r=dv_preference_post[i],allocated_against_ur_r=allocatedAgainstUR,allocated_preference_position_r=i WHERE rollno=candidate_record.rollno;
...
select count(rollno) from candidates where allocated_category_r is not null into count_r;
return master_record.flag_name||' updated successfully for '||count_r||' candidates';						
...
ALTER FUNCTION public.sp_get_allocation_r(text, integer)
    OWNER TO postgres;
