-- FUNCTION: public.sp_get_allocation_f(text, integer)

-- DROP FUNCTION IF EXISTS public.sp_get_allocation_f(text, integer);

CREATE OR REPLACE FUNCTION public.sp_get_allocation_f(
	par_exam_name text,
...
catsel=candidate_record.catsel_f_18_30;		
...
if not (dv_preference_post[i] in ('A01','A02')) then
	continue;
end if;

if (dv_preference_post[i] in ('A01','A02') and candidate_record.cat3 in ('7')) then
	continue;
end if;
   ...
   <<catsel_loop>>
	for j in 1..length_of_catsel
		...
		if(vacancy_record.current>0) then
		  ...
		  UPDATE candidates SET allocated_category_f=catsel_category , allocated_post_f=dv_preference_post[i],allocated_against_ur_f=allocatedAgainstUR,allocated_preference_position_f=i WHERE rollno=candidate_record.rollno;
...
select count(rollno) from candidates where allocated_category_f is not null into count_f;
return master_record.flag_name||' updated successfully for '||count_f||' candidates';						
...
ALTER FUNCTION public.sp_get_allocation_f(text, integer)
    OWNER TO postgres;
